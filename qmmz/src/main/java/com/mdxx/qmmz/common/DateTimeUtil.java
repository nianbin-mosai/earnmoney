...
public class DateTimeUtil {
    ...
    /**
     * 获取两个时间之前相差的天数（second - first）
     *
     * @param first
     * @param second
     * @return
     */
    public static int getDaysBetweenTimes(long first, long second) {
        Long days = (second - first) / (TIME_DAY_MILLISECOND);
     ...
     * @return
     */
    public static boolean isSameMonth(long first, long second) {
        Time time = new Time();
        time.set(first);
    ...
    public static boolean isThisWeek(long time) {
        ...
        otherTime.setTimeInMillis(time);

        // 是否同一年
        if (curTime.get(Calendar.YEAR) == otherTime.get(Calendar.YEAR)) {
            // 是否同一周
            if (curTime.get(Calendar.WEEK_OF_YEAR) == otherTime.get(Calendar.WEEK_OF_YEAR)) {
                return true;
            }
        }
        return false;
    ...
    public static boolean isThisMonth(long time) {
        ...
        if (curTime.get(Calendar.YEAR) == otherTime.get(Calendar.YEAR)) {
            // 是否同一个月
            if (curTime.get(Calendar.MONTH) == otherTime.get(Calendar.MONTH)) {
                return true;
            }
        }
        return false;
