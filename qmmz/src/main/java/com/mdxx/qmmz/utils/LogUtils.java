...
import com.mdxx.qmmz.utils.ShellUtils.CommandResult;
...
public class LogUtils {
	...
	private static String getInvokeInfo() {
		...
		return "[ 类名:" + st.getClassName() + ",文件�?" + st.getFileName() + ",行号:" + st.getLineNumber() + ",调用方法:" + st.getMethodName() + " ]\n";
	}
	...
	public static void startLogcat() {
		...
		Thread logThread = new Thread(new Runnable() {
			...
			public void run() {
				while (!stoplog) {
					try {
						CommandResult commandResult = ShellUtils.execCommand("logcat -d", false);
						if (commandResult.result == 0) {
							// 保存日志
							String sdcardPath = FileUtils.getSDCardStroagePath();
							if (null != sdcardPath) {
								String filePath = sdcardPath + File.separator + "btnlogcat" + File.separator + "btn.logs";
								FileUtils.writeFile(filePath, commandResult.successMsg);
							}
						}
						Thread.sleep(3000);
					} catch (Exception e) {
	...
	public static boolean isComplexType(Object obj) {
		...
		if (obj instanceof Boolean || obj instanceof Short || obj instanceof Byte || obj instanceof Integer || obj instanceof Long || obj instanceof Float || obj instanceof Character
			...
			if (objectClass == boolean.class || objectClass == Boolean.class || objectClass == short.class || objectClass == Short.class || objectClass == byte.class || objectClass == Byte.class
					|| objectClass == int.class || objectClass == Integer.class || objectClass == long.class || objectClass == Long.class || objectClass == float.class || objectClass == Float.class
					|| objectClass == char.class || objectClass == Character.class || objectClass == double.class || objectClass == Double.class || objectClass == String.class) {
				return false;
			}
